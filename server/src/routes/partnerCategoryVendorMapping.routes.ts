...
import { catalogRepository, partnerCategoryVendorMappingRepository, partnerRepository } from '../repositories';
...
partnerCategoryVendorMappingRouter.post('/search', async (req, res, next) => {
  try {
    ...
    const mappings = await partnerCategoryVendorMappingRepository.list({ partnerId, productCategoryId, vendorId, isActive });
 ...
 * partner use for buybacks in this device category?" Returns the active
 * vendor Partner rows directly (not the raw mapping rows) since that's what
 * a caller actually needs next.
 */
partnerCategoryVendorMappingRouter.post('/resolve', async (req, res, next) => {
  try {
    ...
    return res.json({ partnerId, productCategoryId, vendors });
  } catch (err) {
    return next(err);
  }
});
...
partnerCategoryVendorMappingRouter.post('/', async (req, res, next) => {
  try {
    ...
    if (!partnerId || !productCategoryId || !vendorId) {
      return res.status(400).json({ error: 'partnerId, productCategoryId and vendorId are required' });
    }
    ...
    // Not unique on (partnerId, productCategoryId) by design - several
    // vendors can be eligible for the same partner+category - but avoid
    // storing the exact same triple twice.
    const existing = await partnerCategoryVendorMappingRepository.list({ partnerId, productCategoryId, vendorId });
