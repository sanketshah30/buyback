...
export function DiagnosisPage() {
  ...
  useEffect(() => {
    if (!id || isCompleted) return undefined;

    intervalRef.current = setInterval(async () => {
      const updated = await buybackApi.pollDiagnosis(id).catch(() => null);
  ...
  }, [id, isCompleted, setData]);

  useEffect(() => {
    if (isCompleted && id) {
      const timer = setTimeout(() => navigate(`/buyback/${id}/customer`), 1400);
      ...
      <div className="diagnosis-centered">
        ...
        {isCompleted && (
          <>
            ...
            <h2 style={{ marginTop: 12, color: 'var(--color-text)' }}>Diagnosis complete</h2>
            {!!data.diagnosis?.findings?.length && (
              <Banner tone="info">Findings: {data.diagnosis.findings.join(' · ')}</Banner>
            )}
            <Button onClick={() => navigate(`/buyback/${id}/customer`)} style={{ marginTop: 16 }}>
              Continue
            </Button>
          </>
        )}
